...
const CareersSection: React.FC<CareersSectionProps> = ({ translations, onJoinTeam }) => {
  ...
  const jobs = [
    ...
    },
    {
      ...
      loc: translations.careerJob5Loc,
  ...
  const benefits = [
    { icon: "🏠", title: translations.careerBenefitRemote, desc: translations.careerBenefitRemoteDesc },
    { icon: "🎓", title: translations.careerBenefitDev, desc: translations.careerBenefitDevDesc },
  ...
  const handleApply = () => {
    // You might want to pass the selected position to the modal or handle it here
    console.log("Applying for:", selectedPosition);
    onJoinTeam();
  };

  return (
    <section id="careers-segment" className="py-40 px-6 md:px-12 bg-white dark:bg-[#0a1612] overflow-hidden relative">
      {/* Decorative Background Element to fill empty void */}
      ...
      <div className="max-w-[1400px] mx-auto relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-20 items-center mb-32">
          <div className="order-2 lg:order-1 space-y-10">
            ...
            {/* Talent Pulse - Fills the space with actionable stats */}
        ...
        <div id="job-board">
          ...
          <div className="space-y-6">
            {jobs.map((job, i) => (
              <div key={i} className="bg-white dark:bg-dark-serpent/40 rounded-[40px] border border-paper dark:border-green-900/30 overflow-hidden transition-all hover:shadow-2xl hover:border-saffron group">
                <div className="p-10 flex flex-col md:flex-row justify-between items-center">
                  ...
                  <div className="mt-8 md:mt-0 flex items-center gap-8 w-full md:w-auto justify-between md:justify-end">
                    ...
                    <button 
                      ...
                      className="px-8 py-4 border-2 border-castleton-green dark:border-saffron text-castleton-green dark:text-saffron font-black rounded-full hover:bg-castleton-green hover:text-white dark:hover:bg-saffron dark:hover:text-dark-serpent transition-all shadow-lg hover:-translate-y-1 flex items-center gap-2"
                    >
                ...
                <div className={`bg-paper/20 dark:bg-green-900/10 overflow-hidden transition-all duration-500 ease-in-out ${expandedJob === i ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                  <div className="p-10 pt-0 border-t border-paper dark:border-green-900/30">
                    <div className="pt-8">
          ...
          <div className="mt-20 p-10 bg-paper/30 dark:bg-dark-serpent/60 rounded-[40px] border border-paper dark:border-green-900/30 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="w-full md:w-auto">
              <h4 className="text-2xl font-black text-dark-serpent dark:text-white mb-2">{translations.careerApplyTitle}</h4>
              <p className="text-green-2 dark:text-green-4 font-medium">{translations.careerApplyDesc}</p>
            </div>
            <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
              <button 
                onClick={() => onJoinTeam()}
                className="px-10 py-4 bg-castleton-green text-white font-black rounded-full hover:bg-saffron hover:text-dark-serpent transition-all shadow-xl hover:-translate-y-1 whitespace-nowrap"
              >
